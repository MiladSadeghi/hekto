    ...
    <div>
          ...
          <div className='w-full shadow-[0_0_25px_10px_#f6f4fd] flex px-3 py-3 rounded-sm'>
            <div className='w-full flex items-center gap-4'>
              <div className='w-1/4 grid grid-cols-1 gap-2 h-full content-between'>
                {
                  ...
                  productDetails.imagesByColor[productDetails.colors[0].name].map((image: string, index:number) => <img className={`rounded-[4px] duration-100 border-2 border-transparent opacity-70 ease-in-out ${mainImageIndex === index ? "selected-main-image": ""}`} key={image} src={image} alt={productDetails.title} onClick={() => setMainImageIndex(index)} />) :

                  productDetails.images.map((image : string, index:number) => 
                    <img className={`rounded-[4px] duration-100 border-2 border-transparent opacity-70 ease-in-out ${mainImageIndex === index ? "selected-main-image": ""}`} key={image} src={image} alt={productDetails.title} onClick={() => setMainImageIndex(index)} />)
                } 
              </div>
              <div className='w-full'>
                  ...
                  {
                    productDetails.imagesByColor ?
                    <img className='rounded-sm' key={productDetails.imagesByColor[productDetails.colors[0].name][0]} src={productDetails.imagesByColor[productDetails.colors[0].name][mainImageIndex]} alt={productDetails.title} /> :
                    <img className='rounded-sm' key={productDetails.images[0]} src={productDetails.images[mainImageIndex]} alt={productDetails.title} />
                  } 
                
              </div>
            </div>
            <div className='w-full py-16 px-4'>
              <h2 className='text-3xl font-JosefinSans text-[#0D134E] font-bold mb-5'>{productDetails.title}</h2>
              <div>
                {
              ...
              </div>
                {
                  ...
                  (
                    <div className='flex items-center mt-10'>
                      <p className='font-JosefinSans text-navy-blue text-lg font-bold mr-7'>Colors</p>
                      {productDetails.colors.map((item: any) => 
                        <div className="relative mr-2" key={item.code}>
                          
                          <div className={`w-6 h-2 rounded-2xl group cursor-pointer relative`} style={{background: item.code}}>
              ...
              <div className='flex items-center mt-10'>
                <h5 className='font-JosefinSans text-lg font-bold text-navy-blue py-1 px-4 cursor-pointer'>Add To Cart</h5>
                <AiOutlineHeart className='ml-4 font-bold text-navy-blue' size={30} />
              </div>
              <div className='flex mt-10'>
                <p className='mr-7 font-JosefinSans text-lg font-bold text-navy-blue'>Share</p>
      ...
      <div className='bg-[#F9F8FE]'>
        <div className='container mx-auto py-24'>
          <div className='flex'>
            <h6 className={`font-JosefinSans text-2xl font-semibold text-navy-blue ${toggleState === 1? "underline": ""} mr-5`} onClick={(e) => toggleTab(1)}>Specification</h6>
            <h6 className={`font-JosefinSans text-2xl font-semibold text-navy-blue ${toggleState === 2? "underline": ""} mr-5`} onClick={(e) => toggleTab(2)}>Comments</h6>
          </div>
          <div className='mt-10'>
            <div className={`${toggleState === 1? "block": "hidden"} mr-5`}>
              {
                productDetails.specifications.map(specification => (
                  <p key={specification} className="font-JosefinSans text-base font-semibold text-[#A9ACC6] mb-2 last:mb-0 flex items-center group">
