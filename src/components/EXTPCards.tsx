  ...
  return (
    <div className='flex'>
      <div className='bg-[#f5f6f8] min-w-[107px] min-h-[74px] flex items-center justify-center'>
        <img src={data.imagesByColor ? 
          data.imagesByColor[data.colors[0].name][0] :
          data.images[0]
          } alt="" className='w-[64px] h-[71px] object-contain' />
      </div>
      <div className='flex flex-col ml-2 justify-center h-full'>
        <h3 className='line-clamp-1 leading-4 text-base mb-2 w-12/12 text-navy-blue font-semibold'>{data.title}</h3>
        <p className='leading-3 font-JosefinSans text-sm text-navy-blue'>${data.price}</p>
      </div>
    </div>
  )
}
