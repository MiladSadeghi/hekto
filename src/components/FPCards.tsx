  ...
  return (
    <div className='w-[95%] group overflow-hidden'>
      <div className='w-full h-[236px] flex items-center justify-center FPCard-hover-image relative'>
        <img className='w-[176px] h-[176px]' src={data.imagesByColor ? 
      ...
      <div className='flex items-center flex-col p-4 text-center FPCard-hover-body'>
        <p className='line-clamp-1 text-pink-cc text-[18px] font-bold FPCard-hover-text'>{data.title}</p>
        <div className='flex mt-3 h-1'>
          {
            data.colors && 
            data.colors?.map((color: any) => 
              <div key={color.code} className={`w-[14px] h-[4px] mr-2 rounded-lg`} style={{backgroundColor: color.code}} />
            )
          }
        </div>
        <p className='font-JosefinSans text-[14px] text-navy-blue mt-3 FPCard-hover-text'>Code - {data.id}</p>
